...
#must be around 1
overdensity_thresh <- seq(.5,1.5,by=0.1)
...
for(i in seq_along(target_quantiles)) {
  ...
  data$lb <- res$lb <- bounds$lower
  data$ub <- res$ub <- bounds$upper
  ...
  p1 <- ggplot() + 
    ...
    labs(title = "BALLET 2.5%-ile Lower Bound",
         subtitle = paste("noise frac:", target_quantile),
         x = NULL, y = NULL)
  ...
  png(sprintf("output/sky_survey_analysis/edsgc_ballet_pe_%d.png", ceiling(100*target_quantile)),
      width = 5, height = 5, units = 'in', res = 300)
  print(p2)
  dev.off()

  png(sprintf("output/sky_survey_analysis/edsgc_ballet_bounds_%d.png", ceiling(100*target_quantile)),
...
eval <- data.frame(
      sensitivity_edcci =  sensitivity(X, per_clust, edcci),
      specificity_edcci = specificity(X, per_clust, edcci),
      exact_match_edcci = exact_match_frac(X, per_clust, edcci),
      sensitivity_abell =  sensitivity(X, per_clust, abell),
      specificity_abell = specificity(X, per_clust, abell),
      exact_match_abell = exact_match_frac(X, per_clust, abell)
      )

data$per <- per_clust2
enriched_data_pe <- enrich_small_clusters(data, 'per', 
                                          size_lb = 25)
p4 <- ggplot() + 
  geom_point(aes(x = x, y = y, color = per), size = 0.1, data = data %>% filter(per != 0)) + 
  geom_point(aes(x = x, y = y), size = 0.1, alpha = 0.2, color = 'black', data = data %>% filter(per == 0)) +
  stat_ellipse(aes(x = x, y = y, group = per), data = enriched_data_pe %>% filter(per != 0)) +
  geom_point(aes(x = x, y = y, shape = catalogue), size = 2, data = clusters) +
  scale_shape_manual(name = "Cluster\nCatalogue",
                     labels = c("EDCCI", "Abell"),
                     values = c(4,3)) + 
  guides(color = 'none') + 
  labs(title = "BALLET Persistent Clusters and Catalogue Locations",
       subtitle = paste("noise frac:", target_quantile),
       x = NULL, y = NULL)
